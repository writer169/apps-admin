...
export async function POST(request: Request) {
  ...
  console.log('[Login API] JWT_SECRET (first 5 chars, if defined):', process.env.JWT_SECRET?.substring(0, 5)); // ОСТОРОЖНО
  ...
  try {
    ...
    // Проверяем, что переменные окружения для аутентификации загружены
    ...
    // Проверяем, что JWT_SECRET загружен перед генерацией токена
    ...
    response.cookies.set('auth-token', token, {
      ...
      sameSite: 'lax', // Используем 'lax' для большей совместимости
