  ...
  if (pathname.startsWith('/dashboard') || pathname.startsWith('/api/apps') || pathname.startsWith('/api/generate-link')) {
    ...
    if (!payload || !payload.isAdmin) {
      ...
      response.cookies.delete({
        name: 'auth-token',
        path: '/', 
        // Если при установке куки использовались domain, secure, httpOnly, sameSite,
        // их тоже желательно указать здесь для корректного удаления в некоторых браузерах.
        // secure: process.env.NODE_ENV === 'production',
        // httpOnly: true, // httpOnly не указывается при удалении, браузер сам найдет
        // sameSite: 'lax',
      });
